...
const ArticleCard = ({ title, description, category, readTime, views, rating, tags }: ArticleCardProps) => (
  <article className="bg-white border border-border rounded-card p-6 hover:border-brand-red/30 transition-all duration-300 shadow-sm group cursor-pointer flex flex-col">
    <div className="flex items-start justify-between mb-4">
      ...
      <span className="px-3 py-1 bg-brand-navy/10 text-brand-navy text-xs font-medium rounded-pill">
        {category}
      </span>
    </div>

    <h3 className="text-text font-display font-semibold text-lg mb-2 group-hover:text-brand-red transition-colors">
      {title}
    </h3>
    ...
    <div className="flex flex-wrap gap-2 mb-4">
      {tags.map((tag) => (
        <span
          key={tag}
          className="flex items-center gap-1 px-2 py-1 bg-surface text-text-muted text-xs rounded-input"
        >
    ...
    <div className="flex items-center justify-between mt-auto">
      ...
      <ChevronRight className="w-5 h-5 text-text-muted group-hover:text-brand-red transition-colors" />
    </div>
  </article>
);
