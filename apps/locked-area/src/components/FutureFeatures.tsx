...
export const FutureFeatures = ({ title, subtitle, features }: { title: string; subtitle: string; features: FutureFeature[] }) => (
  <section className="container-page py-16">
    <div className="text-center mb-10">
      <span className="text-brand-navy text-sm font-medium uppercase tracking-wider">Vägvisare</span>
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
      {features.map((feature, index) => (
        <div
          key={index}
          className="bg-surface-dark/5 border border-border border-dashed rounded-card p-5 opacity-70 hover:opacity-100 transition-opacity"
        >
